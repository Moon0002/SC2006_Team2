...
import { findTopStoresWithROI } from '@/lib/application/find-nearby-stores'
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {formatTravelTime(roi.travelTimeHours) && (
                        <div className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-100">
                          <span aria-hidden="true">{'\u{1F68C}'}</span>
                          <span>{formatTimeSummary(roi.travelTimeHours, roi.transitData)}</span>
                        </div>
                      )}
                      {formatFareBreakdown(roi.transitData) && (
                        <div className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded bg-purple-50 text-purple-700 border border-purple-100">
                          <span aria-hidden="true">{'\u{1F4B0}'}</span>
                          <span>{formatFareBreakdown(roi.transitData)}</span>
                        </div>
                      )}
                        {store.reviewCount > 0 && (
                          <div className="flex items-center gap-1">
