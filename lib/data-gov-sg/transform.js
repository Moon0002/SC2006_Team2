...
/**
 * NOTE: Item base prices are no longer hardcoded here.
 * We now prefer looking up absolute prices from a Supabase table seeded from
 * `supabase/singstat_data.sql` (loaded by the CPI sync action).
 *
 * This file still keeps a small category fallback list so the pipeline can
 * proceed if an item isn't present in SingStat or the lookup wasn't provided.
 */
...
/**
 * Estimates an SGD price for an item.
 *
 * Priority:
 * 1) If `singstatPriceMap` is provided and contains the item name, use that
 *    absolute price (e.g. Jan 2026).
 * 2) Otherwise fall back to category price scaled by CPI index.
 *
 * @param {number} cpiIndex - CPI index value (base ~100)
 * @param {string} itemName - Item name
 * @param {string} category - Category
 * @param {Map<string, number>|Object<string, number>|null} singstatPriceMap - normalized item -> price
 * @returns {number} Estimated price in SGD
 */
export function estimatePriceFromIndex(
  cpiIndex,
  itemName = '',
  category = 'Other',
  singstatPriceMap = null
) {
  const key = normalizeItemKey(itemName)

  // 1) DB-provided absolute price
  if (singstatPriceMap) {
    const direct =
      singstatPriceMap instanceof Map
        ? singstatPriceMap.get(key)
        : singstatPriceMap[key]
  ...
  // 2) Fallback to category base price scaled by CPI (keeps older behavior)
  const basePrice =
    CATEGORY_FALLBACK_PRICES[category] || CATEGORY_FALLBACK_PRICES['Other']

  const baseIndex = 100.0
  const numericIndex = Number(cpiIndex)
  const safeIndex = Number.isFinite(numericIndex) ? numericIndex : baseIndex

  return (safeIndex / baseIndex) * basePrice
}
 ...
 */
export function transformCpiRecord(record, options = {}) {
  try {
    const { targetMonth = 'latest', singstatPriceMap = null } = options
    ...
    const estimatedPrice = estimatePriceFromIndex(
      monthData.index,
      dataSeries.trim(),
      category,
      singstatPriceMap
 ...
 */
export function transformCpiRecords(records, options = {}) {
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      const transformedRecord = transformCpiRecord(record, options)
