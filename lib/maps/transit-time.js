...
export async function getTransitTravelTimeHours({
  ...
  try {
    ...
    const key = cacheKey(originPostalCode, destinationPostalCode, roundTrip)
    const now = Date.now()
    ...
    url.searchParams.set('origin', `${originPostalCode}, Singapore`)
    url.searchParams.set('destination', `${destinationPostalCode}, Singapore`)
